...
var getBalanceForAssetCmd = &cobra.Command{
	Use:   "get-balance-for-asset",
	Short: "Get asset balance.",
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		request := &intx.GetAssetBalanceRequest{
			PortfolioId: portfolioId,
			AssetId:     utils.GetFlagStringValue(cmd, utils.AssetIdFlag),
		}

		response, err := client.GetAssetBalance(ctx, request)
		if err != nil {
			return fmt.Errorf("cannot get asset balance: %w", err)
		}
